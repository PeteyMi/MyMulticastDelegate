...
#import "MyGCDMulticastDelegate.h"
 ...
 **/
@interface MyGCDMulticastDelegateNode : NSObject {
@private
    
#if __has_feature(objc_arc_weak)
    __weak id _delegate;
#if !TARGET_OS_IPHONE
    __unsafe_unretained id _unsafeDelegate; // Some classes don't support weak references yet (e.g. NSWindowController)
#endif
#else
...
@interface MyGCDMulticastDelegateEnumerator ()
{
...
@interface MyGCDMulticastDelegate ()
{
...
@implementation MyGCDMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    MyGCDMulticastDelegateNode *node = [[MyGCDMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
    ...
    {
        MyGCDMulticastDelegateNode *node = [_delegateNodes objectAtIndex:(i-1)];
        ...
        if (delegate == nodeDelegate)
        {
            ...
            {
                // Recall that this node may be retained by a MyGCDMulticastDelegateEnumerator.
...
{
    for (MyGCDMulticastDelegateNode *node in _delegateNodes)
    {
        node.delegate = nil;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        node.unsafeDelegate = nil;
...
{
    return [[MyGCDMulticastDelegateEnumerator alloc] initFromDelegateNodes:_delegateNodes];
}
...
- (void)forwardInvocation:(NSInvocation *)origInvocation
{
    ...
    if (foundNilDelegate)
    {
        ...
        [_delegateNodes removeObjectsAtIndexes:indexSet];
    }
}
...
@implementation MyGCDMulticastDelegateNode

@synthesize delegate = _delegate;       // atomic
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
@synthesize unsafeDelegate = _unsafeDelegate; // atomic
#endif
@synthesize delegateQueue = _delegateQueue;  // non-atomic
    ...
    if ((self = [super init]))
    {
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        {
            if (SupportsWeakReferences(inDelegate))
            {
                ...
                _delegateQueue = inDelegateQueue;
            }
            else
            {
                _delegate = [NSNull null];
                
                _unsafeDelegate = inDelegate;
                _delegateQueue = inDelegateQueue;
            }
        }
#else
...
@implementation MyGCDMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
    ...
    {
        MyGCDMulticastDelegateNode *node = [_delegateNodes objectAtIndex:_currentNodeIndex];
        _currentNodeIndex++;
